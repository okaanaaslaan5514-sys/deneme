"""Automation controller for Priest Tekli Atak Kırma skill."""
...
from priest.action_scheduler import action_scheduler, PRIORITY_TEKLI
...
@dataclass
class TekliAtakKirmaConfig:
    """Mutable configuration snapshot for Tekli Atak Kırma automation."""

    global_enabled: bool = False
    skill_enabled: bool = False
    primary_key: Optional[str] = None
    function_key: Optional[str] = None
    mode: Optional[str] = None  # "timer" or "malice"
    ...
    def malice_ready(self) -> bool:
        return self.ready() and self.mode == "malice"


class TekliAtakKirmaMicroController:
    """Coordinate Tekli Atak Kırma automation for timer or Malice completion triggers."""

    def __init__(self, status_callback: Callable[[str, int], None]) -> None:
        self._status_callback = status_callback
        self._config = TekliAtakKirmaConfig()
    ...
    def update_config(self, config: TekliAtakKirmaConfig) -> None:
    ...
    def _refresh_timer_thread(self) -> None:
        ...
        self._timer_thread = threading.Thread(target=self._timer_loop, name="TekliAtakTimer", daemon=True)
    ...
    # ------------------------------------------------------------------ #
    # Malice completion trigger
    # ------------------------------------------------------------------ #

    def handle_malice_completion(self) -> None:
        """Invoke Tekli Atak Kırma when Malice automation completes."""
        with self._state_lock:
            config = self._config
        if not config.malice_ready():
            return
        self._execute_sequence(config)
    ...
    def _execute_sequence(self, config: TekliAtakKirmaConfig) -> None:
        if pydirectinput is None:
            if not self._warned_missing_pydirectinput:
                self._status_callback("pydirectinput modülü bulunamadı; Tekli Atak Kırma tetiklenemedi.", 6000)
        ...
        with self._action_lock:
            try:
                with action_scheduler.claim(PRIORITY_TEKLI):
                ...
                self._status_callback(f"Tekli Atak Kırma komutu gönderilemedi: {exc}", 5000)
...
__all__ = ["TekliAtakKirmaConfig", "TekliAtakKirmaMicroController"]
