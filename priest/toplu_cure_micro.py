"""Automation controller for Priest Toplu Cure skill based on icon visibility."""
...
from priest.action_scheduler import action_scheduler, PRIORITY_TOPLU_CURE
...
@dataclass
class TopluCureConfig:
    """Mutable configuration snapshot for Toplu Cure automation."""
...
class TopluCureMicroController:
    """Monitor skill icon and trigger Toplu Cure when it appears."""

    def __init__(self, status_callback: Callable[[str, int], None]) -> None:
        self._status_callback = status_callback
        self._config = TopluCureConfig()
    ...
    def update_config(self, config: TopluCureConfig) -> None:
    ...
    def _refresh_monitor_thread(self, config: TopluCureConfig) -> None:
        ...
        self._monitor_thread = threading.Thread(target=self._monitor_loop, name="TopluCureMonitor", daemon=True)
        ...
        if not self._templates:
            ...
            if not templates:
                if not self._warned_missing_templates:
                    self._status_callback("Toplu Cure için skill görseli bulunamadı. skill_resmi.py ile kayıt yapın.", 6000)
    ...
    def _execute_action(self, config: TopluCureConfig) -> None:
        if pydirectinput is None:
            if not self._warned_missing_pydirectinput:
                self._status_callback("pydirectinput modülü bulunamadı; Toplu Cure tetiklenemedi.", 6000)
        ...
        with self._action_lock:
            if now - self._last_action_ts < MIN_ACTION_GAP:
                return
            try:
                with action_scheduler.claim(PRIORITY_TOPLU_CURE):
                ...
                self._status_callback(f"Toplu Cure komutu gönderilemedi: {exc}", 5000)
                return
            self._last_action_ts = time.monotonic()
        self._status_callback("Toplu Cure tetiklendi.", 1500)
...
__all__ = ["TopluCureConfig", "TopluCureMicroController"]
