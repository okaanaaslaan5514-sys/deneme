"""Automation controller for Priest Subside skill."""
...
from priest.action_scheduler import action_scheduler, PRIORITY_SUBSIDE
F_RETURN_KEY = "f1"
...
CLICK_CENTER_DELAY = 0.5
AFTER_CLICK_DELAY = 1.0
...
@dataclass
class SubsideConfig:
    """Mutable configuration snapshot for Subside automation."""
...
class SubsideMicroController:
    """Trigger Subside on an interval with center click sequence."""

    def __init__(self, status_callback: Callable[[str, int], None]) -> None:
        self._status_callback = status_callback
        self._config = SubsideConfig()
    ...
    def update_config(self, config: SubsideConfig) -> None:
    ...
    def _refresh_timer_thread(self) -> None:
        ...
        self._timer_thread = threading.Thread(target=self._timer_loop, name="SubsideTimer", daemon=True)
    ...
    def _execute_sequence(self, config: SubsideConfig) -> None:
        if pydirectinput is None:
            if not self._warned_missing_pydirectinput:
                self._status_callback("pydirectinput modülü bulunamadı; Subside tetiklenemedi.", 6000)
        ...
        if center is None:
            if not self._warned_missing_pyautogui:
                self._status_callback("pyautogui modülü veya ekran bilgisi alınamadı; Subside tıklaması yapılamadı.", 6000)
        ...
        with self._action_lock:
            try:
                with action_scheduler.claim(PRIORITY_SUBSIDE):
                ...
                self._status_callback(f"Subside komutu gönderilemedi: {exc}", 5000)
    ...
    def _click_center(self, center: Tuple[int, int]) -> None:
        ...
        try:
            ...
            self._status_callback(f"Subside tıklaması başarısız: {exc}", 5000)


__all__ = ["SubsideConfig", "SubsideMicroController"]
