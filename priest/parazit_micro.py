"""Automation controller for Priest Parazit skill with timer or Party MS trigger."""
...
from priest.action_scheduler import action_scheduler, PRIORITY_PARAZIT
...
@dataclass
class ParazitConfig:
    """Mutable configuration snapshot for Parazit automation."""

    global_enabled: bool = False
    skill_enabled: bool = False
    primary_key: Optional[str] = None
    function_key: Optional[str] = None
    mode: Optional[str] = None  # "timer", "partyms", or None
    ...
    def partyms_ready(self) -> bool:
        return self.ready() and self.mode == "partyms"


class ParazitMicroController:
    """Coordinate Parazit automation for timer and Party MS triggers."""

    def __init__(self, status_callback: Callable[[str, int], None]) -> None:
        self._status_callback = status_callback
        self._config = ParazitConfig()
    ...
    def update_config(self, config: ParazitConfig) -> None:
    ...
    def _refresh_timer_thread(self) -> None:
        ...
        self._timer_thread = threading.Thread(target=self._timer_loop, name="ParazitTimer", daemon=True)
    ...
    def handle_partyms_trigger(self) -> None:
        """Invoke Parazit sequence when Party MS action completes."""
        with self._state_lock:
            config = self._config
        if not config.partyms_ready():
            return
        self._execute_sequence(config)
    ...
    def _execute_sequence(self, config: ParazitConfig) -> None:
        if pydirectinput is None:
            if not self._warned_missing_pydirectinput:
                self._status_callback("pydirectinput modülü bulunamadı; Parazit tetiklenemedi.", 6000)
        ...
        with self._action_lock:
            try:
                with action_scheduler.claim(PRIORITY_PARAZIT):
                ...
                self._status_callback(f"Parazit komutu gönderilemedi: {exc}", 5000)
                return
...
__all__ = ["ParazitConfig", "ParazitMicroController"]
