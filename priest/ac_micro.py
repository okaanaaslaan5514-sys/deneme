"""Automation controller for Priest AC skill."""
...
from priest.action_scheduler import action_scheduler, PRIORITY_AC
...
@dataclass
class AcConfig:
    """Mutable configuration snapshot for AC automation."""
...
class AcMicroController:
    """Coordinates AC automation by tracking buff absence."""
    ...
    def update_config(self, config: AcConfig) -> None:
    ...
    def _refresh_auto_thread(self, config: AcConfig) -> None:
        ...
        self._auto_thread = threading.Thread(target=self._auto_loop, name="AcAuto", daemon=True)
        self._auto_thread.start()

    def _prepare_detection_resources(self) -> bool:
        if mss is None:
            if not self._warned_missing_mss:
                self._status_callback("mss modülü bulunamadı; AC otomatik modu devre dışı.", 6000)
    ...
    def _execute_action(self, config: AcConfig) -> bool:
        if not config.primary_key or not config.function_key:
            return False
        if pydirectinput is None:
            if not self._warned_missing_pydirectinput:
                self._status_callback("pydirectinput modülü bulunamadı; AC tetiklenemedi.", 6000)
        ...
        with self._action_lock:
            if now - self._last_action_ts < MIN_ACTION_GAP:
                return False
            try:
                with action_scheduler.claim(PRIORITY_AC):
                ...
                self._status_callback(f"AC komutu gönderilemedi: {exc}", 5000)
                return False
            self._last_action_ts = time.monotonic()
        self._status_callback("AC tetiklendi.", 1500)
        return True
...
__all__ = ["AcConfig", "AcMicroController"]
