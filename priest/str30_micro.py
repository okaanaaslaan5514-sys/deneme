"""Automation controller for Priest STR 30 skill."""
...
from priest.action_scheduler import action_scheduler, PRIORITY_STR30
...
@dataclass
class Str30Config:
    """Mutable configuration snapshot for STR 30 automation."""
...
class Str30MicroController:
    """Coordinates STR 30 automation by tracking buff absence."""

    def __init__(self, status_callback: Callable[[str, int], None]) -> None:
        self._status_callback = status_callback
        self._config = Str30Config()
    ...
    def update_config(self, config: Str30Config) -> None:
    ...
    def _refresh_auto_thread(self, config: Str30Config) -> None:
        ...
        self._auto_thread = threading.Thread(target=self._auto_loop, name="Str30Auto", daemon=True)
        self._auto_thread.start()

    def _prepare_detection_resources(self) -> bool:
        if mss is None:
            if not self._warned_missing_mss:
                self._status_callback("mss modülü bulunamadı; STR 30 otomatik modu devre dışı.", 6000)
    ...
    def _execute_action(self, config: Str30Config) -> bool:
        if not config.primary_key or not config.function_key:
            return False
        if pydirectinput is None:
            if not self._warned_missing_pydirectinput:
                self._status_callback("pydirectinput modülü bulunamadı; STR 30 tetiklenemedi.", 6000)
        ...
        with self._action_lock:
            if now - self._last_action_ts < MIN_ACTION_GAP:
                return False
            try:
                with action_scheduler.claim(PRIORITY_STR30):
                ...
                self._status_callback(f"STR 30 komutu gönderilemedi: {exc}", 5000)
                return False
            self._last_action_ts = time.monotonic()
        self._status_callback("STR 30 tetiklendi.", 1500)
        return True
...
__all__ = ["Str30Config", "Str30MicroController"]
