#!/usr/bin/env python3
"""
Micro script to read HP (health) values from the screen and report percentage.
"""
...
def read_once(image: Optional[Path]) -> str:
    if image:
        ...
        current, maximum = read_bar_from_screen(HP_ROI)
    return format_output(current, maximum)


def main() -> None:
    parser = build_common_parser("HP bar OCR okuyucusu")
    ...
    if args.once or args.from_image:
        try:
            ...
            print(f"HP okunamadı: {exc}", file=sys.stderr)
            sys.exit(1)
        return
    ...
    try:
        for idx in range(limit):
            try:
                ...
                print(f"HP okunamadı: {exc}", file=sys.stderr, flush=True)
            time.sleep(interval)
            if args.from_image:
                break  # image reading doesn't change, so break after one loop
